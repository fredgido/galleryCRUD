...
def makethumb(original):
    image = Image.open(original)
    image.thumbnail(THUMB_SIZE)
    image.convert("RGB").save(os.path.join("static", "thumb", str(os.path.basename(
        original)).split(".")[0] + ".jpg"))
...
def postget():
    ...
    return render_template("post.html", file=post.file, post=post,
                           postdump=json.dumps({"id": post.id, "file": post.file, "text": post.text}, indent=4))
...
def postpoint():
    ...
    text = "" if request.form.get("text") is None else request.form.get("text")
    filename = str(post_id) + "." + request.files.get("file").filename.split(".")[-1]

    file = request.files.get("file", False)
    file.save(os.path.join("static", "orig", filename))  # app.config['UPLOAD_FOLDER']
    makethumb(os.path.join("static", "orig", filename))
...
def new_post():
    ...
    filename = str(post_id) + "." + url.split(".")[-1]

    reply = requests.get(url)
    with open(os.path.join("static", "orig", str(post_id) + "." + url.split(".")[-1]), 'wb') as f:
...
def patch_post():
    ...
    if posted_json['text'] is not None:
        post.text = posted_json['text']

    if posted_json['file'] is not None:
        url = posted_json.get("file")
        post.file = str(posted_json['id']) + "." + url.split(".")[-1]
        reply = requests.get(url)
        with open(os.path.join("static", "orig", post.file), 'wb') as f:
            f.write(reply.content)
        makethumb(os.path.join("static", "orig", post.file))
